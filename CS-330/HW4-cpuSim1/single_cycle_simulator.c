...
// Instruction Opcode
static inline int opcode(int instruction) { return (instruction >> 26) & 0x3F; }

// Dest Register
static inline int field0(int instruction) { return (instruction >> 21) & 0x1F; }

// Src Reg 1
static inline int field1(int instruction) { return (instruction >> 16) & 0x1F; }
...
// Inst Field
static inline int instant(int instruction) { return instruction & 0xFFFF; }

// Instruction Function
static inline int funct(int instruction) { return instruction & 0x3F; }
...
void printInstruction(int instr) {
  ...
  switch (instr_opcode) {
  case ALU:
    printf("%s %d %d %d", instr_funct_str, field0(instr), field1(instr),
           field2(instr));
    break;
  case ADDI:
  case ANDI:
...
#define MAXLINELENGTH                                                          \
  100 // MAXLINELENGTH is the max number of characters we read
